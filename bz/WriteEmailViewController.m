...
- (void)sendItemAction:(UIBarButtonItem *)item
{
    ...
    _sendTask = [NetService POST:kWriteEmailUrl parameters:dict complete:^(id responseObject, NSError *error) {
        ...
        NSLog(@"%@", responseObject);
        if ([responseObject[kStatusCode] integerValue] == NetStatusSuccess) {
            UIAlertView *av = [[UIAlertView alloc] initWithTitle:Localized(@"提示") message:Localized(@"邮件发送成功") delegate:weakSelf cancelButtonTitle:Localized(@"确定") otherButtonTitles:nil, nil];
            [av show];
        } else {
            [Utility showString:responseObject[kErrMsg] onView:weakSelf.view];
        }
